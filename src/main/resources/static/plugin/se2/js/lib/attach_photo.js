    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/board/img/upload'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		// });
		// oAjax.header("Content-Type","multipart/form-data; boundary=XXXX");
		// oAjax.header("file-name",encodeURIComponent(tempFile.name));
		// oAjax.header("file-size",tempFile.size);
		// oAjax.header("file-Type",tempFile.type);
		// oAjax.header("X-CSRF-TOKEN",document.getElementById("csrf").value);
		...
		axios.post(sUploadURL, formData, {
			headers: {
			...
			.then(value => {
				var resultMap = value.data;
				if(resultMap.code === "fail") {
					if (resultMap.message.indexOf("NOTALLOW_") > -1) {
						var sFileName = resultMap.message.replace("NOTALLOW_", "");
						alert("이미지 파일(jpg,gif,png,bmp)만 업로드 하실 수 있습니다. (" + sFileName + ")");
					}
				} else {
					//성공 시에  responseText를 가지고 array로 만드는 부분.
					makeArrayFromString(resultMap.message);
				}
			});
    }
